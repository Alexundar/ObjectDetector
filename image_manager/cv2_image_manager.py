...
from image_manager.abstract_image_manager import AbstractImageManager

class Cv2ImageManager(AbstractImageManager):
    def __init__(self):
        super().__init__()
    ...
    def read_images(self, images):
        list_of_images = []
        try:
            list_of_images = [osp.join(images, img) for img in os.listdir(images)]
    ...
    def write_images(self, image_names, images):
        list(map(cv2.imwrite, image_names, images))
